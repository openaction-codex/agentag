...
- Uses one ephemeral GPT-5.6 Luna call with medium reasoning and a JSON output schema to select Luna/low, Luna/max, Sol/medium, or Sol/xhigh.
...
The webhook posts the initial task card before any Codex call. The preparation worker then calls Codex with `--ephemeral`, `gpt-5.6-luna`, medium reasoning, and `--output-schema`. PR functional validation and technical-specification writing use GPT-5.6 Sol/medium. Other coding work uses GPT-5.6 Sol/xhigh. Extremely simple requests to stop the agent's current work or check the current agent/MCP server status use GPT-5.6 Luna/low. Other simple routine work that does not require long context—including product or general questions, classification, extraction, short summaries, status updates, and simple MCP reads—uses GPT-5.6 Luna/max. Remaining non-coding work uses GPT-5.6 Sol/medium. If selection times out, fails, or returns an invalid route, AgentTag safely falls back to Sol/medium and still queues the task.
